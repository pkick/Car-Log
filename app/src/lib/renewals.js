import { daysBetween, parseISODate, todayISO } from './dates'

/** How many days before its renewal date an insurance or registration renewal counts as coming up. */
 ...
 * @property {'ok' | 'coming-up' | 'overdue'} status `overdue` once the date has passed, `coming-up` from
 *   {@link RENEWAL_WARN_DAYS} days before it through the day itself, `ok` before that
 ...
 * @returns {RenewalItem[]}
 */
export function getRenewalItems(vehicle, policyRecords, today = todayISO()) {
  const items = RENEWAL_TYPES.flatMap(({ type, label, field }) => {
    ...
    const status = daysUntil < 0 ? 'overdue' : daysUntil <= RENEWAL_WARN_DAYS ? 'coming-up' : 'ok'
