...
const HEADERS = [
  'Type', 'Vehicle', 'Date', 'Odometer', 'Gallons', 'Price/Gal', 'Total', 'Station',
  'Category', 'Description', 'Cost', 'Performed By', 'Shop', 'Parts Used', 'Notes',
]

/**
 * One CSV of every fill-up and service record, oldest first, with a header row. Fuel rows carry their station and
 * notes; the CSV import (`lib/csvImport.js`) reads the Fuel rows back.
 * @param {Array<{ id: number, nickname?: string }>} vehicles
 * @param {Array<import('./vehicleStats').FillUp & { vehicleId: number, station?: string | null, notes?: string | null }>} fillUps
...
export function buildCsv(vehicles, fillUps, serviceRecords) {
  ...
  const rows = [
    ...fillUps.map((f) => [
      'Fuel', vehicleName(f.vehicleId), f.date, f.odometer, f.gallons, f.pricePerGal, f.total, f.station,
      '', f.isFull ? 'Fill-up' : 'Partial fill-up', '', '', '', '', f.notes,
    ]),
